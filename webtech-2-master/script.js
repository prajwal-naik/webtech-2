...
app.get('/', function(req, res) {
    ...
    if (name == undefined) {
        ...
        var age = i + '-' + max + '.png'
        res.render('homepage', { name: req.cookies['username'], phone: req.cookies['phone'], address: req.cookies['address'], email: req.cookies['usename'], age: req.cookies['recommend'] })
    }
...
app.post('/homepage', async(req, res) => {
    ...
    try {
        ...
        ref.child(usename1).once("value", function(data) {
            ...
            var mar = ""
            var type = ""
            if (data.val().married === "Married") {
                mar = "yes"
            } else {
                mar = "no"
            }
            if (data.val().occupation === "Private Company / IT Employee" || data.val().occupation === "Business / Self-Employed") {
                type = "type1"
            } else if (data.val().occupation === "Government Employee" || data.val().occupation === "Educational Institute") {
                type = "type2"
            } else {
                type = "type3"
            }
            ...
            age = dt.predict({ age: i, occupation: type, maritialstatus: mar })
            age = age + '-' + max + '.png'
            res.cookie('recommend', age)
    ...
    if (parseInt(otp) == (parseInt(req.cookies['otp']) - 9999)) {
        ...
        try {
            var x = firebase.auth().createUserWithEmailAndPassword(email, password).then(function(record) {
                var mar = ""
                var type = ""
                if (req.cookies['mari'] === "Married") {
                    mar = "yes"
                } else {
                    mar = "no"
                }
                if (req.cookies['occupation'] === "Private Company / IT Employee" || req.cookies['occupation'] === "Business / Self-Employed") {
                    type = "type1"
                } else if (req.cookies['occupation'] === "Government Employee" || req.cookies['occupation'] === "Educational Institute") {
                    type = "type2"
                } else {
                    type = "type3"
                }
